...
fams <- gsub('.posterior.tree','',dir('../data/posteriorTrees'))
...
smallfamilies <- c()
for (fam in fams) {
  trees <- read.tree(paste('../data/posteriorTrees/',fam,'.posterior.tree',sep=''))
...
data.df <- read.csv('../data/charMtx.csv')
all.states <- c()
all.b.lens <- c()
all.parent <- c()
all.child <- c()
...
languages <- read.csv('../data/asjp/languages.csv')
